...
class ListVehicule(ctk.CTkFrame):
    def __init__(self, master, on_add=None, on_select=None, width=1000, height=600):
        super().__init__(master, width=width, height=height, corner_radius=8)
        self.width = width
        self.height = height
        self.on_select = on_select

        self.column_headers = ["ID", "Matricule", "Marque", "Modèle", "Année", "Kilométrage"]
        self.column_attrs = ["ID", "matricule", "marque", "modele", "annee", "kilometrage"]
        ...
        lbl = ctk.CTkLabel(frame, text="📋 Liste des Vehicules", font=("Poppins", 18, "bold"))
        lbl.grid(row=0, column=0, pady=20, padx=20)

        bouton_ajouter = ctk.CTkButton(frame, text="➕ Ajouter Vehicule", command=on_add, width=170, height=35,
                                       font=("Poppins", 14))
